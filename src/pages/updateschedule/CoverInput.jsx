...
import { __coverShow, __imageInfo, __stickerShow } from '../../modules/update';

const CoverInput = () => {
  const dispatch = useDispatch();
  const detail = useSelector((state) => state.schedule.detail);
  const image = useSelector((state) => state.update.image);
  const coverShow = useSelector((state) => state.update.coverShow);
                  ...
                  {imageNum.num === '1' && (
                    <CoverPick1 htmlFor='1' image={image}>
                      <Input
                        ...
                        defaultCheked={imageNum.num === String(detail?.coverImage)}
                      />

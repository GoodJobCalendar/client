  ...
  return (
    <>
      <Head>
        <WeekBtn>&lt;</WeekBtn>
        <WeekTitle>
          {`${String(Month).padStart(2, "0")}월 ${weekIndex}`}주차
        </WeekTitle>
        <WeekBtn>&gt;</WeekBtn>
      </Head>
      <WeekWrap>
        ...
        <DayList>
          {["", "", "", "", "", "", ""].map((elm, idx) => {
            const resultDay = new Date(
              theYear,
              Month,
              Dates + (idx - theDayOfWeek)
            );
            const dd = resultDay.getDate();
            const list = String(dd).length === 1 ? "0" + dd : dd;
            return (
              <li theDayOfWeek={theDayOfWeek} list={list}>
                <span>{list}</span>
              </li>
            );
          })}
...
const DayList = styled.ul`
  ...
  > li {
    ${(props) =>
      props.theDayOfWeek === props.list ? "span{color: var(--blue4)}" : ""};
    padding: 13px 17px;
    width: calc(100% / 7);
    font-weight: 500;
    font-size: 12px;
